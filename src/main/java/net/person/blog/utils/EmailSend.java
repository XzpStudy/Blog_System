...
public class EmailSend {
    ...
    private EmailSend() {
        EmailSend.config(EmailSend.SMTP_QQ(false), "person.user@qq.com", "oqqfdbxctxrwdbah");
    }
     ...
     */
    public static EmailSend subject(String subject) {
     ...
     */
    public EmailSend from(String nickName) {
     ...
     */
    public EmailSend from(String nickName, String from) {
        try {
    ...
    public EmailSend replyTo(String... replyTo) {
    ...
    public EmailSend to(String... to) throws MessagingException {
    ...
    public EmailSend cc(String... cc) throws MessagingException {
    ...
    public EmailSend bcc(String... bcc) throws MessagingException {
    ...
    private EmailSend addRecipients(String[] recipients, Message.RecipientType type) throws MessagingException {
        String result = Arrays.asList(recipients).toString().replace("(^\\[|\\]$)", "").replace(", ", ",");
        msg.setRecipients(type, InternetAddress.parse(result));
        return this;
    }

    private EmailSend addRecipient(String recipient, Message.RecipientType type) throws MessagingException {
        msg.setRecipients(type, InternetAddress.parse(recipient.replace(";", ",")));
        return this;
    }

    public EmailSend text(String text) {
        this.text = text;
        return this;
    }

    public EmailSend html(String html) {
        this.html = html;
        return this;
    }

    public EmailSend attach(File file) {
        attachments.add(createAttachment(file, null));
        return this;
    }

    public EmailSend attach(File file, String fileName) {
        attachments.add(createAttachment(file, fileName));
        return this;
    }

    public EmailSend attachURL(URL url, String fileName) {
    ...
    public static void sendRegisterVerifyCode(String code,String emailAddress) throws MessagingException {
            EmailSend.subject("测试邮件发送")
                    .from("个人博客系统")
                    .text("这是验证码："+code+ "，有效期十分钟，若非本人操作，请忽略此邮件。")
                    .to(emailAddress)
                    .send();
    }
}
